defmodule Proj2.Neighborhood do
  ...
  def get_neighbor(actor_no, num_actors, topology, start_time) do
    if not_empty() do
      case topology do
        ...
        "line" ->
          get_line_neighbor(actor_no, num_actors)

        "imp2D" ->
          get_impline_neighbor(actor_no, num_actors)
      ...
      Proj2.Failure.get_failure_count()
      System.halt(0)
    end
  end
  ...
  def get_full_neighbor(total_actors) do
    ...
    if(check_neighbor_present(rand_neigh)) do
      ...
      Proj2.Failure.update_failure_count()
      get_full_neighbor(total_actors)
    end
  end

  def get_line_neighbor(start_actor, total_actors) do
    cond do
      (start_actor==1) -> # IO.puts "first actor"
                      if(Agent.get(@name, fn map -> Map.has_key?(map, 2) end)) do
                             2
                           else
                              Proj2.Failure.update_failure_count()
                              get_random_neighbor()
      ...
      (start_actor==total_actors) -> # IO.puts "last actor"
                                    if(Agent.get(@name, fn map -> Map.has_key?(map, total_actors-1) end)) do
                                        total_actors-1
                                    else
                                        Proj2.Failure.update_failure_count()
                                        get_random_neighbor()
                                    end
      true -> list = [start_actor+1, start_actor-1]
              rand_neigh = Enum.random(list)
              rand_neigh_list = [rand_neigh]
              list1_list = list -- rand_neigh_list
              list1 = list1_list
              if(!Agent.get(@name, fn map -> Map.has_key?(map, rand_neigh) end)) do
                  if(Agent.get(@name, fn map -> Map.has_key?(map, list1) end)) do
                    list1
                   else
                     find_nearest_line_neighbor(start_actor+2, start_actor-2, total_actors)
                  end
              else
                 rand_neigh
              end
  ...
  def find_nearest_line_neighbor(left, right, total_actors) do
    #This function will traverse, the left and right paths in a line, for eg. if an actor is 5, first it will
    #check for 3 and 6, if any one is present then will go with it, if both are present will select randomly from it and
    #if both are not present will search for 2 and 7 and so on.....

     # IO.puts "check nearest"
    if(left==1 || right==total_actors) do
      # IO.puts "reached dead end"
       Proj2.Failure.update_failure_count()
       get_random_neighbor()
    else
    left_true = check_neighbor_present(left)
    right_true = check_neighbor_present(right)
        case left_true || right_true do
          true  -> if (!(left_true && right_true))
                     do
                       if left_true do left else right end
                   else
                     Enum.random([left, right])
                    end
          false -> find_nearest_line_neighbor(left-1, right+1, total_actors)
        end
      end #if(left==1 || right==total_actors) end
   end #find_nearest_line_neighbor end

  def get_impline_neighbor(start_actor, total_actors) do
    cond do
      (start_actor==1) -> # IO.puts "first actor"
            neigh1 = Enum.random(1..total_actors)
            select = Enum.random([neigh1, 2])
                      if(Agent.get(@name, fn map -> Map.has_key?(map, select) end)) do
                             select+1
                           else
                              Proj2.Failure.update_failure_count()
                              get_random_neighbor()
                             #check_nearest()
                             #find_nearest_line_neighbor(start_actor+2, 1, total_actors)
                        end
      (start_actor==total_actors) -> # IO.puts "last actor"
                  neigh1 = Enum.random(1..total_actors)
                  select = Enum.random([neigh1, start_actor-1])
                                    if(Agent.get(@name, fn map -> Map.has_key?(map, select) end)) do
                                        select-1
                                    else
                                        Proj2.Failure.update_failure_count()
                                        get_random_neighbor()
                                        #find_nearest_line_neighbor(total_actors, start_actor-2, total_actors)
                                    end
      true ->
        neigh1 = Enum.random(1..total_actors)
        list1 = [start_actor+1, start_actor-1, neigh1]
        list2 = [start_actor+1, start_actor-1]
        select = Enum.random(list1)
              rand_neigh = Enum.random(list2)
              if(!Agent.get(@name, fn map -> Map.has_key?(map, select) end)) do
                  if(Agent.get(@name, fn map -> Map.has_key?(map, rand_neigh) end)) do
                    rand_neigh
                   else
